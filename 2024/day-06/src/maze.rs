...
#[derive(Debug, Clone)]
pub struct Lookup(pub(crate) Vec<BTreeSet<usize>>);
...
impl Lookup {
    ...
    #[must_use]
    pub fn new_with(&self, context_index: usize, obstacle: usize) -> Self {
        let mut lookup = self.clone();

        lookup.0[context_index].insert(obstacle);
...
impl Guard {
    pub(crate) const fn new(position: (usize, usize)) -> Self {
        Self {
            position,
